...
5. `tools/audit-initrd.sh` against the fixed official base and `out/systempart-devnode` artifact.
6. A second real derivation compared byte-for-byte with `out/systempart-devnode`.
